...
- ✅ ALWAYS communicate in the user's language unless they request otherwise
...
- 🧩 If file writing is unavailable, continue in chat using the same structure and note the fallback explicitly
...
- List all files in `brainstorming/` under the current working directory
...
"I will facilitate this brainstorming session with structured techniques and keep us in exploration mode long enough to get past the obvious ideas.
...
3. **Do you want a quick session or a deep session?** (If the user is unsure, infer based on urgency and scope and propose one)"
...
- **Session Mode:** [quick mode or deep mode, with one-sentence rationale]
...
session_started_at: '[current timestamp]'
facilitator: '[assistant name if available, otherwise leave blank]'
session_mode: '[quick mode or deep mode]'
---
```
...
When the user confirms the session framing, append the session overview content directly to `{brainstorming_session_output_file}` using the structure from above.
If file writing is unavailable, keep the same content in chat and continue without blocking the session.
...
- Support both file-backed and chat-only execution without losing structure
