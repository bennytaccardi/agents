...
- ✅ ADAPT DEPTH TO `session_mode`: quick mode favors momentum and fewer loops; deep mode favors volume, divergence, and repeated pivots
...
- For option A: go deeper on the selected idea within this session using focused follow-up prompts, assumptions testing, edge-case probing, and concept expansion. If an advanced elicitation skill exists in the environment you may use it, but it is optional and must not be required.
